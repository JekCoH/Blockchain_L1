...
class Blockchain:
    ...
    # Эта функция ниже создана для создания самого первого блока и установки его хэша равным "O"
    def __init__(self):
        self.chain = []
    ...
    def create_block(self, proof, previous_hash):
        ...
        block = {'index': len(self.chain) + 1,
                 ...
                 'user_id': str(df.iloc[len(self.chain), 0]),
                 'user_name': str(df.iloc[len(self.chain), 1]),
                 'age' : str(df.iloc[len(self.chain), 2]),
                 'proof': proof,
